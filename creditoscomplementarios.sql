...
ALTER TABLE Alumno MODIFY COLUMN Carrera VARCHAR(50);

CREATE TABLE Creditos (
    idCredito INT PRIMARY KEY NOT NULL,
    nombreCredito VARCHAR(30) NOT NULL,
    estado VARCHAR(20) NOT NULL,
    CONSTRAINT chk_estado CHECK (estado IN ('APROBADO', 'NO APROBADO'))
);
alter table creditos drop constraint chk_estado;
#alter table creditos modify estado varchar(20) default null;

select * from creditos
CREATE TABLE AlumnoCreditos (
    idAlumno INT,
    idCredito INT,
...
DELETE FROM alumnocreditos;
ALTER TABLE AlumnoCreditos DROP COLUMN cantidadCreditos;

SELECT * FROM AlumnoCreditos WHERE idAlumno = 1 AND idCredito = 1;
